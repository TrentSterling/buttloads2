...
(function (B) {
  ...
  class Combat {
    ...
    targets() { return [...this.enemies, ...(this.foreman?.targets() || []), ...(this.crawlers?.targets() || [])]; }
    hit(n, amount, mode, dir) {
      if (this.crawlers?.owns(n)) return this.crawlers.hit(n, amount, mode, dir);
    ...
    area(p, radius, damage, mode) {
      for (const n of this.targets()) if (n.hp > 0 && distance(n, p) <= radius + .4 && this.world.clearLine(p, n, .05)) {
        ...
        if (n.phase === 'buried' && !this.foreman?.owns(n) && !this.crawlers?.owns(n) && !this.blocked(n)) n.phase = 'idle';
    ...
    update(dt, game) {
      ...
      for (const b of game.gadgets.blasts) if (b.serial > this.lastBlast) {
        ...
        for (let d = 0; d <= (b.length || 0); d += .5) {
          ...
          for (const n of this.targets()) if (!struck.has(n.id) && n.hp > 0 && distance(n, p) <= b.radius + .4 && this.world.clearLine(p, n, .05)) { struck.add(n.id); if (n.phase === 'buried' && !this.foreman?.owns(n) && !this.crawlers?.owns(n) && !this.blocked(n)) n.phase = 'idle'; this.hit(n, 70 * Math.max(.35, 1 - distance(n, p) / (b.radius + 1.2)), 'blast'); }
        }
      }
