...
(function (B) {
  ...
  class View {
    ...
    setDeposits(deposits) {
      ...
      this.ghosts = new T.InstancedMesh(new T.IcosahedronGeometry(.2, 0), new T.MeshBasicMaterial({ transparent: true, opacity: .65, depthTest: false, depthWrite: false, blending: T.AdditiveBlending }), count + 17 + B.MYSTERIES.length + B.THUNDERSTONES.length);
    ...
    render(game, dt, time) {
      ...
      this.renderFeedback(game); this.renderTown(game, dt, time); this.renderCaverns(game); this.renderCombat(game, time); this.renderDeep(game, time); this.renderForeman(game, time);
