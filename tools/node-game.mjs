...
export async function nodeGame() {
  if (!globalThis.B2?.Gadgets) for (const name of ['core', 'town', 'caverns', 'deep-terrain', 'mesher', 'world', 'player', 'ore', 'expedition', 'refuges', 'deep', 'combat', 'foreman', 'rescue', 'crawlers', 'kinetics', 'actions', 'gadgets', 'thunderstone', 'freight', 'mysteries', 'survey', 'persistence', 'feedback', 'audio', 'fieldkit', 'town-ui']) vm.runInThisContext(fs.readFileSync(path.join(root, 'src', name + '.js'), 'utf8'));
  ...
  for (const file of ['render', 'scenery', 'ruins', 'crane', 'feedback-view', 'thunderstone-view', 'town-view', 'cavern-view', 'combat-view', 'deep-view', 'foreman-view', 'rescue-view', 'crawler-view', 'kinetic-view']) vm.runInThisContext(fs.readFileSync(path.join(root, 'src', file + '.js'), 'utf8'));
  ...
  game.play = () => game.setScreen(null); // Explicitly bypass browser input/audio acquisition in simulation.
